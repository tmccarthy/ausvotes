...
object NswLegCoData {
  ...
  class Live[F[+_, +_] : Die : Fs2Compiler](streams: NswLegCoStreams[F]) extends NswLegCoData[F] {
    ...
    private def parseGroupRowFromSpreadsheetRow(election: NswLegCoElection, row: Vector[SpreadsheetCell]): F[Exception, BallotGroup] =
      for {
        ...
        rawParty <- row.lift(2).flatTraverse {
          case SpreadsheetCell.Empty => BMonad.pure(None): F[Exception, Option[String]]
          case cell                  => takeStringFrom(cell).map(Some.apply): F[Exception, Option[String]]
        }
