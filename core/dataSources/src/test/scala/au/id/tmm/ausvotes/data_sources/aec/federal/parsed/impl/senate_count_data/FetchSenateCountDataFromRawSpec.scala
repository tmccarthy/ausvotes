...
class FetchSenateCountDataFromRawSpec extends FlatSpec with NeedsCleanDirectory with OneInstancePerTest {
  ...
  "the ballot position to candidate map" should "be constructed correctly" in {
    val actual = FetchSenateCountDataFromRaw.constructBallotPositionLookup(groupsAndCandidates)
    val expected = Map(
