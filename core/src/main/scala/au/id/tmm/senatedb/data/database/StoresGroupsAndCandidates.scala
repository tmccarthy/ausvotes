...
private[database] trait StoresGroupsAndCandidates { this: Persistence =>
  ...
  def storeGroups(groups: Set[GroupsRow]): Future[Unit] = execute(dal.insertGroups(groups)).map(_ => Unit)

  def storeCandidates(candidates: Set[CandidatesRow]): Future[Unit] = execute(dal.insertCandidates(candidates)).map(_ => Unit)
