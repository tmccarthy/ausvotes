...
class FirstPreferenceCalculatorSpec extends FlatSpec {
  ...
  private val normaliser = BallotNormaliser(CountRules.normalisationRulesFor(election.election), election, candidates)
  ...
  it should "not have a first preference if the first preferenced candidate was independent" in {
    ...
    val ntNormaliser = BallotNormaliser(CountRules.normalisationRulesFor(election.election), election, ntCandidates)
