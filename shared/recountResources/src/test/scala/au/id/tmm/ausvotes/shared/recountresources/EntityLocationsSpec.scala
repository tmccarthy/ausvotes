...
class EntityLocationsSpec extends ImprovedFlatSpec {
  ...
  "the groups object" should "have a location" in {
    assert(
      EntityLocations.locationOfGroupsObject(SenateElection.`2016`.electionsPerState(State.SA)) ===
  ...
  "the candidates object" should "have a location" in {
    assert(
      EntityLocations.locationOfCandidatesObject(SenateElection.`2016`.electionsPerState(State.SA)) ===
  ...
  "the preference tree object" should "have a location" in {
    assert(
      EntityLocations.locationOfPreferenceTree(SenateElection.`2016`.electionsPerState(State.SA)) ===
  ...
  "the canonical recount object" should "have a location" in {
    assert(
      EntityLocations.locationOfCanonicalRecount(SenateElection.`2016`.electionsPerState(State.SA)) ===
