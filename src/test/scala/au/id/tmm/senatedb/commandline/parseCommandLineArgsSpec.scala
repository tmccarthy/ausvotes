...
class parseCommandLineArgsSpec extends ImprovedFlatSpec {
  ...
  it should "default to UNSPECIFIED" in {
    ...
    assert(result.right.get.verb === Verb.UNSPECIFIED)
  }
  ...
  it should "parse LOAD" in {
    ...
    assert(result.isRight, result)
    assert(result.right.get.verb === Verb.LOAD)
  }
  ...
  it should "parse RELOAD" in {
    ...
    assert(result.isRight, result)
    assert(result.right.get.verb === Verb.RELOAD)
  }
  ...
  it should "default to the rawData directory" in {
    ...
    assert(result.isRight, result)
    assert(result.right.get.rawDataDirectory === Paths.get("rawData"))
  }
  ...
  it should "be specified with the --rawData flag" in {
    ...
    assert(result.isRight, result)
    assert(result.right.get.rawDataDirectory === Paths.get("/tmp/rawData"))
  }
  ...
  it should "default to a file at senateDB.db" in {
    ...
    assert(result.isRight, result)
    assert(result.right.get.sqliteLocation === Some(Paths.get("senateDB.db")))
  }
  ...
  it should "be specified with the --sqlite flag" in {
    ...
    assert(result.isRight, result)
    assert(result.right.get.sqliteLocation === Some(Paths.get("/tmp/db.db")))
  }
  ...
  it should "default to false" in {
    ...
    assert(result.isRight, result)
    assert(!result.right.get.forbidDownload)
  }
  ...
  it should "default to the 2016 election" in {
    ...
    assert(result.isRight, result)
    assert(result.right.get.election === SenateElection.`2016`)
  }
  ...
  they should "be specified without flags" in {
    ...
    assert(result.isRight, result)
    assert(result.right.get.statesToLoad === Set(State.NSW, State.VIC, State.SA))
  }
  ...
  they can "all be requested by the --allStates flag" in {
    ...
    assert(result.isRight, result)
    assert(result.right.get.statesToLoad === State.ALL_STATES)
  }
