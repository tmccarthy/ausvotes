...
object BallotsUsingCrossesReportGenerator extends ReportGenerator with TallyReportGenerator {
  override private[reporting] def shouldCount(ballotWithFacts: BallotWithFacts): Boolean = {
    PreferenceDetectionUtilities.containsPreference(ballotWithFacts.ballot, Preference.Cross)
  }
}
