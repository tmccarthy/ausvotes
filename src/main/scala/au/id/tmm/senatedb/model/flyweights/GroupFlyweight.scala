...
final class GroupFlyweight private () {
  private val flyweight: Flyweight[(SenateElection, State, String, Option[Party]), Group] = Flyweight {
    case (election, state, code, party) => Group(election, state, code, party)
  }

  def apply(election: SenateElection, state: State, code: String, party: Option[Party]) =
